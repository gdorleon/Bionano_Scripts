...
if [[ $1 == "bnxstats" ]] && [[ $# -gt 1 ]]; then
	...
	echo
        ...
        printf "Filtred Data(100kb)""\t""Number of Molecules""\t""Total Lenght Mb""\t""N50 kb""\t""Avg Intensity""\t""Lab Density/100kb""\t""Average SNR""\n">$outDir/CSV/Header100kb.csv

 (echo "$(basename ${bnx})	${n_mol}	${cL}	`awk "BEGIN{ print ${n50}/1000}"`	${avgL}	`awk "BEGIN{print ${den}/${cL}/10}"`	${avgSNR}">>$outDir/CSV/Filtr100kb.csv)

# percent calculator

 printf "Total Lenght Mb""\n"> $outDir/CSV/HFilter100.csv
 (echo "${cL}">> $outDir/CSV/PFilter100.csv) 
 
 cat $outDir/CSV/HFilter100.csv $outDir/CSV/PFilter100.csv > $outDir/CSV/NFilter100.csv
 rm -rf $outDir/CSV/HFilter100.csv $outDir/CSV/PFilter100.csv 
